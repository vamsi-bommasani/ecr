...
func TestTerraformVPC(t *testing.T) {
	...
	out, errOut, err := run(ctx, repoRoot, "terraform", "output", "-json", "repository_url")
	if err != nil {
		t.Fatalf("terraform output failed: %v\nstdout: %s\nstderr: %s", err, out, errOut)
	}

	// terraform output -json repository_url returns a JSON string
	var repoURL string
	if err := json.Unmarshal([]byte(out), &repoURL); err != nil {
		t.Fatalf("failed to parse repository_url output: %v\noutput: %s", err, out)
	}

	if repoURL == "" {
		t.Fatalf("expected repository URL output, got empty string")
	}

	// Log the created repository URL
	t.Logf("Created Repository URL: %s", repoURL)
}
